...
//!
//! Usage:
//!     cargo run --bin inject_fake_gps_mlg -- <input.mlg> [output.mlg]
...
// Closed-loop trace centered on Воробьёвы горы (Moscow). Oval shape so it
// reads obviously as "a track" on the map, with multiple laps per minute so
// the lap detector finds something interesting.
const CENTER_LAT: f64 = 55.7100;
const CENTER_LON: f64 = 37.5500;
const RADIUS_LAT_M: f64 = 250.0;
const RADIUS_LON_M: f64 = 450.0;
const LAP_PERIOD_S: f64 = 45.0;
...
fn main() -> ExitCode {
    ...
    println!(
        "  trace: oval centered at ({:.4}, {:.4}) radii {}m x {}m, {}s/lap",
        CENTER_LAT, CENTER_LON, RADIUS_LAT_M, RADIUS_LON_M, LAP_PERIOD_S
    );
...
///
/// Drives an oval loop around `(CENTER_LAT, CENTER_LON)` with period
/// `LAP_PERIOD_S`. Phase advances slightly with `record_index` to break
/// perfect repeatability across loops, mimicking real GPS jitter.
fn synth_lat_lon(t_s: f64, record_index: usize) -> (i32, i32) {
    let phase = 2.0 * PI * (t_s / LAP_PERIOD_S);
    let jitter = (record_index as f64).sin() * 0.5; // ±0.5 m
    let dlat_m = RADIUS_LAT_M * phase.cos() + jitter;
    let dlon_m = RADIUS_LON_M * phase.sin() + jitter;
